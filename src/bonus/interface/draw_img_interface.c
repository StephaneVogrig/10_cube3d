...
/*   Updated: 2025/01/24 03:03:45 by aska             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_images(t_interface *interface, t_map *map)
{
    ...
    interface->img_map = mlx_new_image(interface->mlx, map->width * TILES_W, map->height * TILES_H);
    if (interface->img_map == NULL)
        return (FAIL);
    draw_image_minimap(interface, start, vector2i(map->width * TILES_W, map->height * TILES_H), map);
    return (SUCCESS);
}
