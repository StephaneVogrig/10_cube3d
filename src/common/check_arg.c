...
/*   Updated: 2025/01/31 21:37:47 by aska             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_entry_arg(int ac, char **av)
{
	...
	if (ext == NULL || ft_strcmp(ext, ".cub") != 0)
		return (ft_return(ERROR, FAIL, "123No \".cub\" extension map file", av[1]));
	root_path = ft_strrchr(av[1], '/');
	if (root_path != NULL && root_path[1] == '.')
		return (ft_return(ERROR, FAIL, "No \".cub\" extension map file", av[1]));
	return (SUCCESS);
}
