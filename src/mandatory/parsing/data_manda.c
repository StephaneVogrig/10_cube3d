...
int	mlx_setup(t_data *data, t_assets *textures)
{
	int	exit_code;
	...
	exit_code |= asset_load_to_buffer(data->mlx, &textures->south, tex_path->so);
...
int	data_setup(t_data *data, char *map_path)
{
	...
	exit_code = lstmap_extract_info(&data->textures, &data->map, map_path);
	if (exit_code == SUCCESS)
		exit_code = map_checker(&data->map, &data->player);
	if (exit_code == SUCCESS)
		exit_code = mlx_setup(data, &data->textures);
	return (exit_code);
}
