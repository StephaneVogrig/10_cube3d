...
/*   Updated: 2025/02/03 21:33:33 by svogrig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static
void	draw_ceil_floor(t_window *win, void *img, t_rgb color_ceil,
						t_rgb color_floor)
{
	int	x;
	int	y;
	...
	while (y < win_h_2)
	{
		...
		while (x < win->width)
		{
			mlx_set_image_pixel(win->mlx, img, x, y, color_ceil.integer);
			mlx_set_image_pixel(win->mlx, img, x, y + win_h_2,
				color_floor.integer);
			x++;
		}
		y++;
...
int	floor_ceil_init(t_window *win, t_rgb color_ceil,
					t_rgb color_floor, t_floor_ceil *mlx_imgs)
{
