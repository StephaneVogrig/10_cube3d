...
/*   Updated: 2025/01/11 17:44:22 by aska             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_image_bg(t_interface *interface, t_vec2i start, t_vec2i end)
{
    ...
    while (y < end.y)
    {
        ...
        while (x < end.x)
        {
            ...
            if (color != 0xFF000000)
                mlx_set_image_pixel(interface->mlx, interface->img_bg, x, y, color);
            x++;
        }
        y++;
    }
...
void	draw_image_fg(t_interface *interface, t_vec2i start, t_vec2i end)
{
    ...
    while (y < end.y)
    {
        ...
        while (x < end.x)
        {
            ...
            if (color != 0xFF000000)
                mlx_set_image_pixel(interface->mlx, interface->img_fg, x, y, color);
            x++;
        }
        y++;
    }
...
int	draw_images(t_interface *interface, t_map *map)
{
    ...
    start = vector2i(0, 0);
	end = vector2i(720, 480);
    interface->img_bg = mlx_new_image(interface->mlx, 720, 480);
    if (interface->img_bg == NULL)
        return (FAIL);
    draw_image_bg(interface, start, end);
    interface->img_fg = mlx_new_image(interface->mlx, 720, 480);
    if (interface->img_fg == NULL)
        return (FAIL);
    draw_image_fg(interface, start, end);
    interface->img_map = mlx_new_image(interface->mlx, map->width * 24, map->height * 24);
    if (interface->img_map == NULL)
        return (FAIL);
    draw_image_minimap(interface, start, vector2i(map->width * 24, map->height * 24), map);
    return (SUCCESS);
}
