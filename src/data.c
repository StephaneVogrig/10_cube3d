...
/*   Updated: 2024/09/23 22:15:55 by stephane         ###   ########.fr       */
/*                                                                            */
/******************************************************************************/
...
int	file_load(char *path, t_data *data)
{
	...
	if (texture_setup(&data->map.textures.north, data->mlx, "maps/test/ROCKNO.bmp") == FAIL)
		return (FAIL);
	if (texture_setup(&data->map.textures.south, data->mlx, "maps/test/ROCKSO.bmp") == FAIL)
		return (FAIL);
	if (texture_setup(&data->map.textures.east, data->mlx, "maps/test/ROCKEA.bmp") == FAIL)
		return (FAIL);
	// if (texture_setup(&data->map.textures.west, data->mlx, "maps/test/ROCKWE.bmp") == FAIL)
	// 	return (FAIL);
	if (texture_jpg_setup(&data->map.textures.west, data->mlx, "maps/test/svogrig_42.jpeg") == FAIL)
		return (FAIL);
	// printf("width: %i Height: %i\n", data->map.width, data->map.height);
	// printf("texture north width: %i Height: %i\n", data->map.tex_north.width, data->map.tex_north.height);
