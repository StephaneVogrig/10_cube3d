...
/*   Updated: 2025/02/04 23:49:21 by ygaiffie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	option_get_resolution(t_option *option, char *resolution)
{
	...
	while (*resolution != '\0' && *resolution != 'x')
		resolution++;
	if (*resolution == '\0')
		return (ft_return(ERROR, FAIL, "Invalid width in resolution", resolution));
	*resolution = '\0';
	resolution++;
	if (*resolution == '\0')
		return (ft_return(ERROR, FAIL, "Invalid height in resolution", resolution));
	height = resolution;
	if (is_valid_resolution(width) == FALSE
		|| is_valid_resolution(height) == FALSE)
		return (ft_return(ERROR, FAIL, "Invalid resolution", resolution));
